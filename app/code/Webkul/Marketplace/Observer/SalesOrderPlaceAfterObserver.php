...
class SalesOrderPlaceAfterObserver implements ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        if (!$isMultiShipping) {
            ...
            $this->_marketplaceHelper->logDataInLogger('Order Data: ' . json_encode($order->getData()));
    ...
    public function orderPlacedOperations($order, $lastOrderId)
    {
        ...
        foreach ($sellerOrder as $info) {
                ...
                $order->formatPrice($this->_marketplaceHelper
                    ->getCurrentCurrencyPrice($currencyRate, $totalTaxAmount)) . '</span></td>
                                ...
                                <tr class='subtotal'>
                                    ...
                                    <td colspan='3'><span>" .
                $order->formatPrice($this->_marketplaceHelper->getCurrentCurrencyPrice(
                    $currencyRate,
                    $totalprice +
                        $totalTaxAmount +
                        $shippingCharges +
            ...
            // only check items belonging to THIS seller
            ...
            foreach ($sellerItems as $sellerItem) {
                try {
                    $product = $this->_productRepository->getById($sellerItem->getMageproductId());
                } catch (\Exception $e) {
                    continue;
                }
            ...
            $emailTempVariables['has_ffl']    = $hasFfl;
            $emailTempVariables['has_normal'] = $hasNormal;

            // ================= SELLER-WISE ORCHID RESTRICTION FOR EMAIL =================

            $sellerId = (int) $info['seller_id'];

            // Get order item IDs for this seller
            ...
            $productRestrictions  = []; // [productName => [messages...]]
                    ...
                    if (json_last_error() === JSON_ERROR_NONE && is_array($decoded)) {
                        ...
                        if (!empty($decoded['restriction'])) {
                            foreach ((array) $decoded['restriction'] as $code) {
                                $productRestrictions[$name][] =
                                    $code . ' - ' . ($restrictionMessages[$code] ?? $code);
                            }
                        ...
                        if (
                            !empty($decoded['shipping_restriction'])
                            && isset($shippingMessages[$decoded['shipping_restriction']])
                        ) {
                            $code = $decoded['shipping_restriction'];
                            $productRestrictions[$name][] =
                                $code . ' - ' . $shippingMessages[$code];
                        ...
                        // Age restriction
                        if (
                            !empty($decoded['age_restriction'])
                            && isset($ageMessages[$decoded['age_restriction']])
                        ) {
                            $code = $decoded['age_restriction'];
                            $productRestrictions[$name][] =
                                $code . ' - ' . $ageMessages[$code];

                            $hasRestrictedProducts = true;
                        }

                        continue; // JSON handled
                    }

                    // ---------- SINGLE / NUMERIC RESPONSE ----------
                    if ($level !== '' && isset($restrictionMessages[$level])) {

                        $productRestrictions[$name][] =
                            $level . ' - ' . $restrictionMessages[$level];
            ...
            $emailTempVariables['has_restricted_products'] = $hasRestrictedProducts;
            $emailTempVariables['restricted_message'] = '';
            ...
            if ($hasRestrictedProducts) {
                ...
                foreach ($productRestrictions as $productName => $messages) {
                    ...
                    $output[] =
                        ...
                        implode('<br/>', array_unique($messages));
                }

                $emailTempVariables['restricted_message'] =
                    '<strong>This order contains restricted item(s) and has been placed on hold for additional verification.</strong><br/><br/>' .
                    implode('<br/><br/>', $output);
            }
